...
<div class="flex flex-col items-center w-full mt-8">
    <div class="flex flex-col w-full pt-12">
        {#each Object.entries(iconSets) as [name, iconSets], index}
            {#if (selectedCategory === null || selectedCategory === index) && (iconSets.filter(iconSet => !iconFilterTerm.trim() || iconSet.name.toLowerCase().includes(iconFilterTerm.trim().toLowerCase()))).length}
                ...
                <div class="flex-wrap p-4 md:p-12 grid gap-4 grid-cols-[repeat(auto-fit,minmax(300px,1fr))] sm:grid-cols-[repeat(auto-fit,minmax(400px,1fr))]">
                    {#each iconSets as iconSet}
                        ...
                        <div class="flex flex-col sssm:flex-row flex-grow overflow-hidden rounded-lg shadow-md w-full">
                            <div class="flex flex-col flex-shrink-0 text-2xl font-medium tracking-wide text-white w-full sssm:w-36">
                                <div class="bg-{colors[index]}-400 text-white w-full h-full flex items-center justify-center px-4 py-3 gap-5 sssm:gap-3">
                                    {#each iconSet.sample_icon.split(',') as sampleIcon}
                                        <Icon icon={`${iconSet.prefix}:${sampleIcon}`} class="w-9 h-9 sssm:w-8 sssm:h-8"/>
                                    {/each}
                                </div>
                                <div class="w-full h-full bg-{colors[index]}-500 flex px-4 py-2 sssm:py-0 justify-between items-center text-white" style="font-size: 1.2rem">
                            ...
                            <div class="flex flex-col justify-between px-4 py-3 w-full">
                                <a href="./icon-set/{iconSet.prefix}" class="text-{colors[index % 7]}-400 w-11/12 whitespace-nowrap overflow-ellipsis overflow-hidden tracking-wide text-2xl font-medium">{iconSet.name}</a>
                                <p class="mt-3 overflow-hidden text-lg tracking-wide text-gray-400 whitespace-nowrap overflow-ellipsis w-11/12">By <a target="_blank" href={iconSet.url || '/'} class="text-{colors[index % 7]}-400">{iconSet.author}</a></p>
                            </div>
                        </div>
                        {/if}
                    {/each}
