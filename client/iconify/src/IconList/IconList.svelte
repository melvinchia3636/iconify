<script>
    ...
    async function getIconSet(tag, isNewTag) {
        ...
        currentTag = tag || null
        const res = await fetch(`http://localhost:3001/fetch-iconlist/${icon_set}/${page}?${tag ? "tag=" + tag.replace("&", "%26") : ""}${searchTerm ? "&q=" + searchTerm.toLowerCase() : ""}`)
        const data = await res.json()
        iconlist = data.icons.length ? (!isNewTag ? iconlist.concat(data.icons) : data.icons) : []
        version = data.version || "1.0.0"
        name = data.name
        icon_count = data.icon_count
        tags = data.tags
...
<div class="w-full px-24 flex flex-col justify-center">
    ...
    <div class="bg-white shadow-md inline-flex items-center mx-auto p-4 mb-6 gap-4 w-full rounded-md overflow-hidden">
        ...
        <input bind:value={searchTerm} on:input={() => getIconSet(currentTag, true)} type="text" class=" tracking-wide text-xl text-gray-500 w-full" placeholder="Search {icon_count} icons (Press '/' to focus)" />
    </div>
            ...
            <div class="w-full grid gap-4 pb-8" style="grid-template-columns: repeat(auto-fill, minmax(200px, 1fr))">
                {#each iconlist as icon}
                    <div class="shadow-md rounded-md flex flex-col gap-12 p-12 pb-4 items-center justify-between">
                        <Icon icon={`${icon_set}:${icon.name}`} width="56" height="56" class="text-gray-700" />
    ...
    {/if}
    {#if hasMore}
        <button on:click={() => {page ++; isLoading = true; getIconSet()}} class="bg-blue-500 font-medium flex items-center justify-center text-white text-xl rounded-md shadow-md px-12 h-16 tracking-wide">
            {#if isLoading}
            <div
            class="-mb-12">
                <LottiePlayer
